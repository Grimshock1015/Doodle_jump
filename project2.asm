...
display_dimension:   .word 32         # display pixels
platform_width:      .word 8          # width of a platform
frame_time:     .word 40 
...
jump_height:     .word 12      # max jump duration in frames
score:           .word 0
...
main:
        ...
        add $s1, $zero, $s0         # $s1 is the current memory position for drawing
        # Initialize draw flag
        add $s5, $zero, $zero       # 0 == draw platforms, 1 == skip drawing
...
check_each_platform:
	bge $t5, $t7, apply_fall # if index > number of platforms = fall
	mul $t9, $t5, $t8        # offset = i * 16 bytes
	add $t9, $t6, $t9        # $t9 = address of current platform[i]
	
	lw $s1, 0($t9)           # $s1 = platform row
	lw $s2, 4($t9)           # $s2 = platform column

	
	bne $s1, $t3, next_plat  # if doodle_row != plat row
	li $s3, 8                # platform width = 6 units
	add $s4, $s2, $s3        # $s4 = plat_col + 6
	addi $s2, $s2, -2	 # considering the feet is 3 size
	blt $t2, $s2, next_plat  # platform left <= doodle <= platform right
	bge $t2, $s4, next_plat 
	
	la $s5, jump_height
	lw $s6, 0($s5)
	sw $s6, 0($a0)           # jump_counter = jump_height
	j done_gravity
...
draw_character_doodle:
	...
	# Check if col == 31 → left on edge
	li $t4, 31
	beq $a1, $t4, left_on_edge
	
   	# Check if col == 30 → middle on edge
	li $t4, 30
...
draw_platforms:
	...
	li $t4, 32                # screen width in units
